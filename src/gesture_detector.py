...
class GestureDetector:
    """
    Adaptive pinch detection with EMA smoothing and hysteresis.
    Detects gestures based on 2D normalized image landmarks.
    ...
    def __init__(
        ...
        # Minimum time finger distance must remain closed for a press
        self.min_press_ms: int = int(gd.get("min_press_ms", gd.get("drag_hold_threshold_ms", 60)))
        ...
        # Ignore trivial closings
        self.min_close_ratio: float = float(gd.get("min_close_ratio", 0.25))

        # Internal states per pinch pair
        self.left = _PinchState()  # Index (8) + Middle (12)
        self.right = _PinchState() # Middle (12) + Ring (16)
        
        # Middle-click = both left and right pairs pinched
        self.middle_last_ts: float = 0.0
        self.middle_debounce_ms: int = int(gd.get("click_debounce_ms", 120))

        # Distance floor for baseline updates (ignore noisy distances)
        self._baseline_update_floor = 0.03 # 3% of screen width
    ...
    def detect(self, landmarks: Sequence[Sequence[float]]) -> Optional[str]:
        ...
        landmarks: iterable of 21 points [x, y, z?] in normalized [0..1]
                   (uses 2D image landmarks)
        Returns one of: "LEFT_CLICK", "RIGHT_CLICK", "MIDDLE_CLICK", or None
        """
        now = time.perf_counter()

        # Pair distances (using 2D normalized landmarks)
        ...
        # Edge-triggered left/right clicks
        if self._edge_click(self.left, ratio_left, now):
            return "LEFT_CLICK"

        if self._edge_click(self.right, ratio_right, now):
            return "RIGHT_CLICK"
    ...
    def _edge_click(self, st: _PinchState, ratio: Optional[float], now: float) -> bool:
        ...
        if st.is_pinched and st.press_started_ts > 0:
            ...
            if held_ms >= self.min_press_ms and since_last_ms >= self.click_debounce_ms:
                # One-shot click on edge
                st.last_click_ts = now
                st.press_started_ts = 0.0 # Prevent re-fire until release
                return True
        return False